...
import { Youtube, FileText, CheckSquare, MessageCircle, Trophy, Gamepad2, History, Gift, ShoppingBag, Calendar, Settings, ChevronLeft, Home, Menu, X, Bot, BarChart3, BrainCircuit, Award, CreditCard, Mail } from 'lucide-react';
...
export const FloatingDock: React.FC<Props> = ({ onTabSelect, onGoHome, onGoBack, isStudent, settings }) => {
    ...
    const menuItems: { id: StudentTab | 'SUB_HISTORY' | 'AI_HISTORY' | 'PRIZES' | 'ANALYTICS', icon: any, label: string, color: string }[] = [
        { id: 'AI_CHAT', icon: Bot, label: 'AI Tutor', color: 'text-indigo-600' },
        { id: 'INBOX', icon: Mail, label: 'Inbox', color: 'text-sky-500' },
        ...
        { id: 'ANALYTICS', icon: BarChart3, label: 'Analytics', color: 'text-teal-600' },
        { id: 'AI_HISTORY', icon: BrainCircuit, label: 'AI History', color: 'text-violet-600' },
        { id: 'LEADERBOARD', icon: Trophy, label: 'Rank', color: 'text-yellow-600' },
        { id: 'PRIZES', icon: Award, label: 'Prizes', color: 'text-amber-600' },
        { id: 'GAME', icon: Gamepad2, label: 'Game', color: 'text-orange-600' },
        { id: 'HISTORY', icon: History, label: 'History', color: 'text-slate-600' },
        { id: 'SUB_HISTORY', icon: CreditCard, label: 'My Plan', color: 'text-green-600' },
        ...
        if (item.id === 'ANALYTICS' && settings?.dashboardLayout?.['tile_analytics']?.visible === false) return false;
        if (item.id === 'AI_HISTORY' && settings?.dashboardLayout?.['tile_ai_history']?.visible === false) return false;
        if (item.id === 'PRIZES' && settings?.dashboardLayout?.['tile_prizes']?.visible === false) return false;
        if (item.id === 'SUB_HISTORY' && settings?.dashboardLayout?.['tile_my_plan']?.visible === false) return false;
